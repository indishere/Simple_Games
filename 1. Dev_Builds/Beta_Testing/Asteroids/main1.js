...
// debt rocks (added from the other version)
let debtRockX = [];
let debtRockY = [];
let debtRockSize = [];
let debtRockXSpeed = [];
...
// gameplay
// NOTE: using 'money' instead of 'score' to match debt logic
let money = 0;
let lives = 3;
let iFrames = 0;
let gameOver = 0;
let bankrupt = 0; // numeric timer like gameOver
...
function keyStuff() {
  // keep your "simple" control vibe: F/S/E plus arrows
  right = keyIsDown(70) || keyIsDown(RIGHT_ARROW); // F or right
  left = keyIsDown(83) || keyIsDown(LEFT_ARROW);  // S or left
  boost = keyIsDown(69) || keyIsDown(UP_ARROW);   // E or up
...
function moveShip() {
  ...
  // reset local turn flags like original
  right = false;
  left = false;
}
      ...
      for (let j = 0; j < rockCount; j++) {
        if (rockSize[j] > 0) {
          ...
          if (dist < (10 + rockSize[j]) / 2) {
            laserVis[i] = false;
            rockSize[j] -= 10;
            money += floor(random(5, 100)); // give money for destroying rocks
            rockRegen(j);
          }
        }
      }
...
function moveDebtRock() {
  for (let i = 0; i < debtRockCount; i++) {
    ...
    if (debtRockX[i] < 0) {
      debtRockX[i] = windowWidth;
      debtRockRegen(i);
    }
    if (debtRockX[i] > windowWidth) {
      debtRockX[i] = 0;
      debtRockRegen(i);
    }
    if (debtRockY[i] < 0) {
      debtRockY[i] = windowHeight;
      debtRockRegen(i);
    }
    if (debtRockY[i] > windowHeight) {
      debtRockY[i] = 0;
      debtRockRegen(i);
    }
  }
    ...
    if (laserVis[i]) {
      for (let j = 0; j < debtRockCount; j++) {
        if (debtRockSize[j] > 0) {
          ...
          let dist = sqrt(dx * dx + dy * dy);
          if (dist < (10 + debtRockSize[j]) / 2) {
            laserVis[i] = false;
            debtRockSize[j] -= 10;
            money += floor(random(5, 100)); // money for clearing debt too
            debtRockRegen(j);
          }
        }
      }
...
function hitDebtRock() {
  for (let i = 0; i < debtRockCount; i++) {
    if (debtRockSize[i] > 10) {
      ...
      let dist = sqrt(dx * dx + dy * dy);
      if (dist < (30 + debtRockSize[i]) / 2) {
        money -= 100;
        if (money < -2000 && bankrupt == 0) {
          bankrupt = 100;
        }
        iFrames = 100;
        xSpeed = -xSpeed;
